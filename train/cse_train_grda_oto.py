...
if __name__ == '__main__':
    ...
    parser.add_argument('--ts', type=int, default=2)
    parser.add_argument('--gpu', type=int, default=-1, help='using gpu' )
    args = parser.parse_args()

    args.save = '{}/SAVED_LOGS/TRAIN-S{}-S{}-SUB{}-AM-ConvBLS-{}-{}'.format(
        Project_Path, args.ss, args.ts, args.sub, args.dataset, time.strftime("%Y%m%d-%H%M%S"))
    ...
    if args.dataset == 'SEED-IV' and args.ss == 1 and args.ts == 2:
        Searched_Path = '{}/SAVED_LOGS/XXX'.format(Project_Path)
    elif args.dataset == 'SEED-V' and args.ss == 1 and args.ts == 2:
        Searched_Path = '{}/SAVED_LOGS/XXX'.format(Project_Path)
    else:
        raise ValueError('ERROR')

    best_result = train_grda(
            train_loader=train_loader_list[args.sub-1],
