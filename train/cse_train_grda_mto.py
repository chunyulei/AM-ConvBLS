...
if __name__ == '__main__':
    ...
    parser.add_argument('--ts', type=int, default=3)
    parser.add_argument('--gpu', type=int, default=-1, help='using gpu' )
    args = parser.parse_args()

    args.save = '{}/SAVED_LOGS/CSE-MTO-TRAIN-TS{}-SUB{}-ConvBLS-DAG-{}-{}'.format(
        Project_Path, args.ts, args.sub, args.dataset, time.strftime("%Y%m%d-%H%M%S"))
    ...
    train_loader1 = train_loader_list1[args.sub-1]
    train_loader2 = train_loader_list2[args.sub-1]
    ...
    if args.dataset == 'SEED-IV' and args.ts == 1:
        Searched_Path = '{}/SAVED_LOGS/XXXX'.format(Project_Path)
    elif args.dataset == 'SEED-IV' and args.ts == 2:
        Searched_Path = '{}/SAVED_LOGS/XXXX'.format(Project_Path)
    elif args.dataset == 'SEED-IV' and args.ts == 3:
        Searched_Path = '{}/SAVED_LOGS/XXXX'.format(Project_Path)
    elif args.dataset == 'SEED' and args.ts == 3:
        Searched_Path = '{}/SAVED_LOGS/XXXX'.format(Project_Path)
    elif args.dataset == 'SEED' and args.ts == 2:
        Searched_Path = '{}/SAVED_LOGS/XXXX'.format(Project_Path)
    else:
    ...
    best_result = train_grda(
            ...
            test_loader=test_loader_list[args.sub-1],
            num_class=args.num_class,
            device=device,
            save=args.save,
            seed=args.seed,
            model_path='{}/saved_models/sub{}'.format(Searched_Path, args.sub)
    )

    logging.info('---------------------------------------------AM-ConvBLS------------------------------------------------------')
    logging.info('| SUBJECT={} | BEST_C={} | BEST_LAM1={} | BEST_LAM2={} | TEST_ACC={:>7.3f}% | TEST_F1={:>7.3f}% | TEST_MCC={:>7.3f}% |'
                 .format(args.sub, best_result['c'], best_result['lambda1'], best_result['lambda2'], best_result['acc'], best_result['f1'], best_result['mcc']))
    logging.info('--------------------------------------------------------------------------------------------------------------')
