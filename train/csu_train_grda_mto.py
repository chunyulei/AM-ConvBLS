...
if __name__ == '__main__':
    ...
    parser.add_argument('--sess', type=int, default=2, help='session id(1, 2, 3)')
    parser.add_argument('--ts', type=int, default=0, help='target domain subject(0-15 for seed-v,0-14 for seed-iv)')
    parser.add_argument('--NoTD', type=int, default=4, help='Number of Source Domains')
    parser.add_argument('--gpu', type=int, default=-1, help='using gpu' )
    args = parser.parse_args()

    args.save = '{}/SAVED_LOGS/TRAIN-MTO-SESS-{}-TS-{}-ConvBLS-DAG-{}-{}'.format(
    ...
    for tar_data, tar_labels in test_loader:
        print('number of target domain samples{}'.format(len(tar_data)))
    target_data = tar_data.flatten() 
    ...
    logging.info('selected source subject index:{}'.format(top_subjects))
    ...
    print('number of source domain samples {}'.format(len(train_dataset)))
    ...
    if args.dataset == 'SEED-V' and args.sess == 1:
        ...
        Searched_Path = '{}/SAVED_LOGS/XXX'.format(Project_Path)
    elif args.dataset == 'SEED' and args.sess == 2:
        Searched_Path = '{}/SAVED_LOGS/XXX'.format(Project_Path)
    else:
    ...
    best_result = train_grda(
            ...
            save=args.save,
            seed=args.seed,
            model_path='{}/saved_models/sub{}'.format(Searched_Path, args.ts+1)
    )
